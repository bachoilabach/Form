...
export const fetchSurvey = async (): Promise<SurveyResponse[]> => {
  try {
    const res = await axios.get<any, SurveyResponse[]>('http://10.10.112.229:3001/surveys');
    const { data } = res;
    return data;
  } catch (error: any) {
    Toast.show({
...
export const submitSurveys = async (
  ...
  try {
    const res = await axios.post<SurveyResponse>('http://10.10.112.229:3001/surveys', survey);
    const { data } = res;
    return data;
  } catch (error: any) {
    Toast.show({
